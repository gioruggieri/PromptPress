...
const normalizeOmmlTextNodesDom = (root: XmlEl) => {
  // Normalizza SOLO i nodi <m:t> per evitare:
  // - &nbsp; visibile in Word
  // - caratteri invisibili resi come "quadratino"
  traverseElements(root, (el) => {
    if (!isM(el, "t")) return;

    let t = el.textContent ?? "";

    // entità HTML rimaste come testo
    t = t.replace(/&nbsp;|&#160;|&#xA0;/gi, " ");
    ...
    // Invisible operators / joiners / zero-width ecc.
    t = t.replace(/[\u200B\u200C\u200D\uFEFF\u00AD\u2060-\u2064]/g, "");

    // controlli non ammessi
    t = t.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, "");

    // rimuovi pseudo-markup finito nel testo (tag spezzati/escapati)
    // NB: non rimuoviamo '<' in generale: può essere un simbolo matematico.
    t = t.replace(/<\/?m:[^>]+>/g, "");

    // preserva spazi significativi
    if (/^\s|\s$| {2,}/.test(t)) {
      const has = el.getAttribute("xml:space");
      if (!has) el.setAttribute("xml:space", "preserve");
    }

    // xmldom: assegna textContent per riscrivere il text node
...
const removeNonMathElements = (root: XmlEl) => {
  // Rimuove elementi non-OMML (in particolare <w:rPr/>) che Word interpreta come corruzione.
  const toRemove: XmlEl[] = [];
  traverseElements(root, (el) => {
    if (el.tagName === "root") return;

    // elimina esplicitamente prefissi noti
    if (/^(w|r|wp|w14|w15|w16|o|v|mc):/.test(el.tagName)) {
      toRemove.push(el);
      return;
    }

    // conserva sempre m:* anche se namespaceURI è vuoto in alcune build
...
const repairInvalidScripts = (root: XmlEl) => {
  // Unwrap di script invalidi per evitare OMML non conforme.
...
const promoteDelimitersInContainer = (container: XmlEl) => {
  ...
  // Include anche il caso "one-sided" tipico di cases: { <eqArr>  (senza delimitatore destro oppure con ".").
  ...
  const createDelimiterWithOptionalEnd = (doc: XmlDoc, beg: string, end: string, inner: XmlEl) => {
    ...
    const endChr = doc.createElementNS(M_NS, "m:endChr") as unknown as XmlEl;
    endChr.setAttribute("m:val", end); // end può essere "" per delimitatore destro assente
  ...
  while (changed) {
    ...
    for (let i = 0; i < nodes.length; i += 1) {
      ...
      // Caso standard: coppia completa open + tall + close
      if (closeChar) {
      ...
      // Caso one-sided: tipico di \begin{cases} ... \end{cases}
      // - openChar è un delimitatore di apertura
      // - segue un costrutto "tall" (eqArr/matrix/fraction/...)
      // - NON c'è un closeChar corrispondente oppure c'è un "." (equivalente di \right.)
      if (!begOnly.has(openChar)) continue;

      const isRightDot = closeChar === ".";
      const hasOtherClose = !!closeChar && closeChar !== ".";

      // Se c'è un carattere di chiusura diverso da ".", non forzare: potrebbe essere testo legittimo.
      if (hasOtherClose) continue;

      const d = createDelimiterWithOptionalEnd(
        container.ownerDocument as unknown as XmlDoc,
        openChar,
        "", // niente delimitatore destro
        b,
      );
      ...
      // Se c'era un "." usato come \right., rimuovilo
      if (isRightDot && c) container.removeChild(c as unknown as XmlNode);
...
const fixEmptyNaryIntegrand = (root: XmlEl) => {
  // Fix pratico per casi tipo: ∫_a^b <integrando> dx
  // Alcuni converter emettono <m:nary> con <m:e/> vuoto e l'integrando come sibling successivo.
  traverseElements(root, (el) => {
...
const postProcessOmml = (omml: string): string | null => {
  ...
  // 0) elimina elementi non-OMML (es. w:rPr)
  removeNonMathElements(root);

  // 1) ripara integrali con <m:e/> vuoto
  fixEmptyNaryIntegrand(root);

  // 2) promuovi delimitatori stretchy
  traverseElements(root, (el) => {
    if (isM(el, "t")) return;
    promoteDelimitersInContainer(el);
  });

  // 3) ripara script incompleti
  repairInvalidScripts(root);

  // 4) rimuovi m:sty undefined
  removeUndefinedStyle(root);

  // 5) normalizza testo (nbsp, invisibili, controlli)
...
const buildOmmlFromElement = (el: HTMLElement): string | null => {
  ...
  // Preferisci il MathML di KaTeX quando non ci sono casi “critici”
...
const inlineFromNodes = (
  ...
  for (const node of nodes) {
    ...
    // evita di duplicare l'output KaTeX (usiamo MathML/annotation)
...
const patchDocxWithMath = async (fileBuffer: Buffer, replacements: MathReplacement[]) => {
  ...
  // Sostituzione robusta: ogni token dovrebbe stare in un singolo <w:r>.
  // Se un token fosse spezzato in più run, va forzato a monte (TextRun unico, senza splitting).
  for (const { token, omml } of replacements) {
    ...
    // Inserisci OMML direttamente (no <w:r>, no escaping)
